...
Tensor dsmm_hb(const Tensor& a_dense, const SparseTensor& b_sparse) {
  ...
  TORCH_CHECK(a_dense.is_hammerblade(), "Sddmm: expected 'a' to be a HammerBlade tensor");
  TORCH_CHECK(b_sparse.is_hammerblade(), "Sddmm: expected 'b' to be a HammerBlade tensor");
  ...
  TORCH_CHECK(b_sparse.sparse_dim() == 2, "We do not support hybrid sparse tensor for 'b' (sparse) in HammerBlade sddmm!");
